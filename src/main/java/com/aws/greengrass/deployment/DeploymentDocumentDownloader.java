...
public class DeploymentDocumentDownloader {
    ...
    private String thingName;   // not final as it could be dynamically changed


    /**
     * Constructor.
     *
     * @param greengrassServiceClientFactory greengrassServiceClientFactory
     * @param deviceConfiguration            deviceConfiguration
     * @param httpClientProvider             httpClientProvider
     */
    @Inject
    public DeploymentDocumentDownloader(GreengrassServiceClientFactory greengrassServiceClientFactory,
                                        DeviceConfiguration deviceConfiguration,
                                        HttpClientProvider httpClientProvider) {
        this.greengrassV2DataClient = greengrassServiceClientFactory.getGreengrassV2DataClient();
        deviceConfiguration.getThingName().subscribe((why, newv) -> thingName = Coerce.toString(newv));
    ...
    private String downloadFromUrl(String deploymentId, String preSignedUrl)
        ...
        logger.atInfo().kv("DeploymentId", deploymentId)
                .log("Making HTTP request to the pre signed url");  // url is not logged for security concerns
        ...
        try (SdkHttpClient client = httpClientProvider.getSdkHttpClient()) {
            ...
            try {
                ...
                throw new DeploymentDocumentDownloadException("I/O error when making HTTP request with pre signed url.",
                        e);
            }
    ...
    private GetDeploymentConfigurationResponse getDeploymentConfiguration(String deploymentId)
        ...
        try {
            ...
            deploymentConfiguration =
                    greengrassV2DataClient.getDeploymentConfiguration(getDeploymentConfigurationRequest);
    ...
    private void validate(String preSignedUrl, String algorithm, String digest)
            throws DeploymentDocumentDownloadException {
        // TODO rethink
        Validate.notBlank(preSignedUrl, "preSignedUrl can't be null or blank");
        Validate.notBlank(algorithm, "algorithm can't be null or blank");
        Validate.notBlank(digest, "digest can't be null or blank");
